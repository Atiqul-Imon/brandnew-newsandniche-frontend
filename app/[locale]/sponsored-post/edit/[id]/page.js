...
export default function SponsoredEditPage({ params }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await api.get(`/api/sponsored-posts/${id}`, { headers: { 'X-Edit-Token': token || '' } });
  ...
  const saveDraft = async () => {
    ...
    setMessage('');
    try {
      const res = await api.put(`/api/sponsored-posts/${id}/status`, { status: 'under_review', adminNotes: '', content: { en: html } }, { headers: { 'X-Edit-Token': token || '' } });
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-4">Edit Sponsored Draft</h1>
