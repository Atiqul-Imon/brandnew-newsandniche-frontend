...
import { Box, Paper, Table, TableHead, TableRow, TableCell, TableBody, Chip, IconButton, Snackbar, Alert, Button, Typography } from '@mui/material';
import CheckIcon from '@mui/icons-material/Check';
import CloseIcon from '@mui/icons-material/Close';
import PublishIcon from '@mui/icons-material/Publish';

export default function GuestSubmissionsPage({ params }) {
  const { locale } = params;
  ...
  const fetchData = async () => {
    try {
      const res = await api.get('/api/guest-posts');
      setRows(res.data?.data?.submissions || []);
    } catch (e) {
      setSnackbar({ open: true, message: 'Failed to load guest submissions', severity: 'error' });
    }
  };
  ...
  const act = async (id, action, body = {}) => {
    try {
      if (action === 'approve') {
        await api.put(`/api/guest-posts/${id}/status`, { status: 'approved' });
      } else if (action === 'reject') {
        await api.put(`/api/guest-posts/${id}/status`, { status: 'rejected', rejectionReason: 'Not a fit' });
      } else if (action === 'publish') {
        await api.post(`/api/guest-posts/${id}/publish`);
      }
  ...
  return (
    <Box>
      <Typography variant="h4" sx={{ mb: 2 }}>Guest Submissions</Typography>
      <Paper>
        <Table>
          <TableHead>
            <TableRow>
          ...
          <TableBody>
            {rows.map((r) => (
              <TableRow key={r._id}>
                ...
                <TableCell>{new Date(r.submissionDate).toLocaleDateString('en')}</TableCell>
