...
export default function EditBlogPage() {
  ...
  useEffect(() => {
    async function fetchBlog() {
      try {
        const res = await axios.get(`http://localhost:5000/api/blogs/${locale}/${params.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/blogs/${locale}/${params.id}`, formData, {
        headers: {
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            {saving ? t('common.saving') : t('blog.update')}
          </button>
        </div>
      </form>
    </div>
